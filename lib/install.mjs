 ...
 *       "skills": { "include": ["specific-skill"], "exclude": [] },
 *       "fullDepth": false
 *     }
 *   ]
 * }
...
async function processEntry(entry) {
  ...
  try {
    ...
    // Normalize skills filter: array shorthand → { include: [...], exclude: [] }
    const { include: skillInclude = [], exclude: skillExclude = [] } =
      Array.isArray(skillsFilter)
        ? { include: skillsFilter, exclude: [] }
        : skillsFilter;

    // Apply include filter
    if (skillInclude.length > 0 && !skillInclude.includes("*")) {
      const includeSet = new Set(skillInclude.map((s) => s.toLowerCase()));
      skills = skills.filter((s) => includeSet.has(s.name.toLowerCase()));
    }

    // Apply exclude filter
    if (skillExclude.length > 0) {
      const excludeSet = new Set(skillExclude.map((s) => s.toLowerCase()));
      skills = skills.filter((s) => !excludeSet.has(s.name.toLowerCase()));
    }
